...
def Main_NMF(NumberOfHotspots,ListOfSites,ListOfCrimeTypes,ListOfDates,DataMin,DataMax,dataset,crimetypes,ListOfMonths,ListOfDays,ListOfPeriods):
	...
	dic_Month=GenerateDic_Labels(ListOfMonths)
	dic_Day=GenerateDic_Labels(ListOfDays) 
	dic_Period=GenerateDic_Labels(ListOfPeriods) 
	...
	for row in listOfCrimes:
		...
		if(isInRange(DataMin,DataMax,ActualData)  and (crimeInterm==crimetype.lower() or crimetype=="")):
			...
			[nameMonth,nameDay,periodDay]=GetLabelofDate(ActualData)
			dic_Month[nameMonth]+=1
			dic_Day[nameDay]+=1
			dic_Period[periodDay]+=1
	...
	for key,value in dic_Month.items():
		dic_Month[key]/=numberOfCrimes

	for key,value in dic_Day.items():
		dic_Day[key]/=numberOfCrimes

	for key,value in dic_Period.items():
		dic_Period[key]/=numberOfCrimes
	...
	return {"snmf":respuesta,'labels':respuestaLabels,'DSite':Dhotspot_Result,'Dcrimetype':Dcrimetype_Result,'Months':dic_Month,'Days':dic_Day,'Periods':dic_Period}
